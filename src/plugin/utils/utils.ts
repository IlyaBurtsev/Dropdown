...
function isTouchOnTargetByClassName(event: BrowserEvent, classNames: Array<string>, currentTouch: Touch): boolean {
  const target: HTMLElement = currentTouch.target as HTMLElement;
  let result = false;
...
function checkTouch(event: BrowserEvent, targetElement: HTMLElement): boolean | Touch {
  if (event.type.indexOf('touch') === 0) {
    const touches = Array.prototype.filter.call(event.touches, isTouchOnTargetByClassName);
...
function checkTouchByClassName(event: BrowserEvent, classNames: Array<string>): boolean | Touch {
  if (event.type.indexOf('touch') === 0) {
